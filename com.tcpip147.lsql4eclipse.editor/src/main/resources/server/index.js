...
var SyntaxError = class extends Error {
  ...
  column;
  cursor;
  constructor(expected, line, column, cursor) {
    super("Syntax error: Expected " + expected + " at line " + line + ", column " + column);
...
var LsqlFile = class {
  ...
  anchor = 0;
  queryList = [];
  constructor(str, isContent = false) {
    if (isContent) {
      this.content = str.replace(/\r/g, "");
    } else {
      this.content = import_fs.default.readFileSync(str, "utf8").replace(/\r/g, "");
    }
    this.parse();
  }
  parse() {
    while (!this.isEOF()) {
      const query = new LsqlQuery();
      this.queryList.push(query);
      this.expectIdSymbol();
      this.expectWhitespace();
      const id = this.parseIdValue();
      query.id = id;
      if (this.isEOF()) {
        break;
      }
      this.expectLinebreak();
      while (this.predictOptionalSymbol()) {
        const optionalKey = this.parseOptionalSymbol();
        if (optionalKey === "description") {
          if (this.predictWhitespace()) {
            this.expectWhitespace();
            const optionalValue = this.parseOptionalValue();
            query.description = optionalValue;
            if (this.isEOF()) {
              break;
            }
            this.expectLinebreak();
      ...
      const sql = this.parseSql();
      query.sql = sql;
    }
  }
  throwSyntaxError(expected, cursor) {
    const lines = this.content.slice(0, cursor).split("\n");
    const line = lines.length;
    const column = lines[lines.length - 1].length;
    throw new SyntaxError(expected, line, column, cursor);
  }
  isEOF(predict = false) {
    if (predict) {
      return this.anchor >= this.content.length;
    } else {
      return this.cursor >= this.content.length;
    }
  }
  getChar(predict = false) {
    if (predict) {
      return this.content[this.anchor];
    } else {
  ...
  expectIdSymbol() {
    ...
    for (let i = 0; i < symbol.length; i++) {
      if (this.getChar() !== symbol[i]) {
        this.throwSyntaxError(symbol, this.anchor);
      }
      this.cursor++;
    }
  }
  ...
  expectWhitespace() {
    ...
    if (this.anchor === this.cursor) {
      this.throwSyntaxError("' '", this.anchor);
    }
  }
  expectLinebreak() {
    this.anchor = this.cursor;
  ...
  parseIdValue() {
    ...
    do {
      ...
      c = this.getChar().charCodeAt(0);
      this.cursor++;
    } while (this.isLetterOrDigit(c) || c == 46 || c == 95);
    this.cursor--;
    const id = this.content.slice(this.anchor, this.cursor);
    if (id.trim().length == 0) {
      this.throwSyntaxError("<@id value>", this.anchor);
    }
    return id;
  }
  predictOptionalSymbol() {
    this.anchor = this.cursor;
    if (this.getChar(true) === "@") {
      this.anchor++;
    } else {
    ...
    let c;
    do {
      const ch = this.getChar(true);
      if (ch == null) {
        break;
      }
      c = this.getChar(true).charCodeAt(0);
      this.anchor++;
  ...
  parseOptionalSymbol() {
    ...
    this.cursor--;
    if (this.cursor < this.anchor + 2) {
      this.throwSyntaxError("<@optional key>", this.anchor);
    }
    return this.content.slice(this.anchor + 1, this.cursor + (this.isEOF() ? 0 : 0));
  }
  predictEmptyLine() {
    this.anchor = this.cursor;
    while (this.getChar(true) === " " || this.getChar(true) === "	" || this.getChar(true) === "\n") {
      if (this.getChar(true) === "\n") {
        return true;
      }
      this.anchor++;
  ...
  expectEmptyLine() {
    ...
    if (!pass) {
      this.throwSyntaxError("<empty line>", this.anchor);
    }
  }
  parseOptionalValue() {
    this.anchor = this.cursor;
    ...
    return this.content.slice(this.anchor, this.cursor);
  }
  parseSql() {
    this.anchor = this.cursor;
    const nextIdSymbol = this.content.indexOf("@id", this.anchor);
    const startOfSql = this.anchor;
    if (nextIdSymbol === -1) {
      this.cursor = this.content.length;
      return this.content.slice(startOfSql);
    } else {
      let endSql;
      for (let i = nextIdSymbol - 2; i >= 0; i--) {
        if (this.content[i] === "\n") {
          this.cursor = i + 1;
          endSql = i;
          break;
      ...
      this.expectEmptyLine();
      this.cursor = nextIdSymbol;
      return this.content.slice(startOfSql, endSql);
    }
  }
};
...
documents.onDidOpen((event) => {
  const { languageId, uri } = event.document;
  if (languageId === "lsql") {
...
async function validateDocument(document) {
  return new Promise((resolve) => {
    ...
    const diagnostics = [];
    try {
      const file = new LsqlFile(text, true);
    } catch (e) {
      if (e instanceof SyntaxError) {
        diagnostics.push({
          severity: import_node.DiagnosticSeverity.Error,
          range: {
            start: document.positionAt(e.cursor),
            end: document.positionAt(e.cursor)
          },
          message: e.message,
          source: "lsql-language-server"
        });
        resolve(diagnostics);
      }
    }
